...
def has_bab(aba, seq_list):
	...
	"""
	for seq in seq_list:
		if any(map(lambda x: seq[x] == aba[1] == seq[x + 2] and seq[x + 1] == aba[0], range(len(seq) - 2))):
			return True
	return False
...
def get_abas(seq):
	...
	"""
	return map(lambda x: seq[x:x+3], filter(lambda x: seq[x] != seq[x + 1] and seq[x] == seq[x + 2], range(len(seq) - 2)))
...
def read(string):
	...
	return (re.findall('(?<=\[)[a-z]+(?=\])', string), re.findall('(?<=\])[a-z]+|[a-z]+(?=\[)', string))
