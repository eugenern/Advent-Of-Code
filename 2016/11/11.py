...
def new_state(state, elevator, direction, *items):
    """
    return the state of the floors given items to move and direction to move in
    (expressed as the value of new floor - current floor)
    """
    new = deepcopy(state)
    for item in items:
...
def fried(state):
    ...
    for floor in state:
        for item in floor:
            if item[-1] == 'M' \
                and correspondings[item] not in floor \
