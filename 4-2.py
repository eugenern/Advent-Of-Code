...
def check(name, checksum):
	...
	for c in name:
		if c in all_letters:
			...
			all_letters[c] = 0
	count = 0
	for letter_freq in sorted(sorted(all_letters.items(), key=lambda x: x[0]), key=lambda x: x[1], reverse=True):
...
def read(string):
	...
	"""
	return (re.match('([a-z\-]+)-\d', string).group(1), int(re.search('[0-9]+', string).group(0)), re.search('\[([a-z]+)\]', string).group(1))
...
def solve(reader, writer):
	...
	writer a writer
	"""
	writer.write(str([read(line)[1] for line in reader if 'northpole' in decrypt(read(line)[0], read(line)[1]) and check(read(line)[0], read(line)[2])])) # calling read() 5 times inefficient, oh well
