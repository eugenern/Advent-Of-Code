"""
Given boss stats, determine the least amount of mana the player can use to defeat the boss on hard mode
"""
...
def run_turns(state, boss_dmg):
    ...
    """
    # first, player loses 1 hp on hard mode
    state['player_hp'] -= 1
    if state['player_hp'] <= 0:
        return []

    # then, apply any active effects and decrement their timers
    apply_effect(state)
    if state['boss_hp'] <= 0:
        return [state]
