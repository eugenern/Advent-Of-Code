"""
Given a list of ingredients and their properties per teaspoon,
find the best score possible for 100 teaspoons of ingredients
"""
...
def calc_score(props, amounts):
    ...
    #     => max(0, 44 * -1 + 56 * 2) * ...
    return prod(max(0, sum(t)) for t in zip(*[[a * p for p in props[i]] for i, a in enumerate(amounts)]))
...
def parse(line):
    ...
    """
    tokens = line.rstrip('.\n').split()
    cap, dur, fla, tex = (int(tokens[tokens.index(prop) + 1].rstrip(',')) for prop in ('capacity', 'durability', 'flavor', 'texture'))
