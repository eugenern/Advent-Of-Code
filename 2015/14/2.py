"""
Given a list of reindeer speeds and flying/rest periods, find who has the most points after 2503s
"""
...
def solve(filepath, end_time):
    ...
    reindeer_dists_points = {reindeer: [0, 0] for reindeer in reindeer_stats}
    for time in range(end_time):
        for reindeer, (speed, fly_time, rest_time) in reindeer_stats.items():
            cycle_len = fly_time + rest_time
            if time % cycle_len < fly_time:
                reindeer_dists_points[reindeer][0] += speed
        farthest = max(dp[0] for dp in reindeer_dists_points.values())
        for dp in reindeer_dists_points.values():
            if dp[0] == farthest:
                dp[1] += 1

    return max(dp[1] for dp in reindeer_dists_points.values())
