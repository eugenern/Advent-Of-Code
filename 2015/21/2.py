"""
Given an item shop and boss stats, find the most expensive set of items to lose to the boss
"""
...
def solve(filepath, player_stats):
    ...
    for items in product(weapons, armor, chain.from_iterable(combinations(rings, r) for r in range(3))):
        ...
        if not battle(tuple(map(sum, zip(player_stats, (0, total_dmg, total_arm)))), boss_stats) and total_cost > worst_found:
            worst_found = total_cost
    return worst_found
