"""
Given a list of package weights, find grouping such that all packages are in one of 3 groups and
...
def solve(filepath):
    ...
    found_valid = False
    best_qe_found = 0
    for i in range(1, len(weights) // 3 + 1):
        for combo in combinations(weights, i):
            if sum(combo) == group_weight:
                remaining_weights = weights.difference(combo)
                for j in range(i, len(remaining_weights) // 2 + 1):
                    for combo_2 in combinations(remaining_weights, j):
