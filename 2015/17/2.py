...
def solve(filepath, goal):
    ...
    valids = [combo for combo in chain.from_iterable(combinations(containers, i) for i in range(start, end + 1)) if sum(combo) == goal]
    min_containers = min(map(len, valids))
    return len([valid for valid in valids if len(valid) == min_containers])
