"""
Given a list of distances between locations, calculate the total distance of the longest route
...
def solve(reader):
    """
    Use distances between locations to find longest route to visit every location once
    """
    # idea: form a dict start loc -> dict dest loc -> distance
    # remember to set dict item in both directions

    # also, during the search (BFS or DFS?) keep track of most total distance found so far
    ...
    # permutations of routes from there, subtract the least distance in the path
