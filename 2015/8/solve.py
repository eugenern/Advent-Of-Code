...
if __name__ == "__main__":
    ...
    raw_len = sum(map(len, raw_lines))
    print(raw_len - sum(len(decode(line, 'unicode-escape')) - 2 for line in raw_lines))
    print(sum(len(line) + line.count('\\') + line.count('"') + 2 for line in raw_lines) - raw_len)

    raw_samp_lines = RAW_SAMPLE.split()
    raw_samp_len = sum(map(len, raw_samp_lines))
    # print(raw_samp_len)
    # for line in raw_samp_lines:
    #     print(f'\t{len(line)}')
    print(raw_samp_len - sum(len(decode(line, 'unicode-escape')) - 2 for line in raw_samp_lines))
    print(sum(len(line) + line.count('\\') + line.count('"') + 2 for line in raw_samp_lines) - raw_samp_len)
